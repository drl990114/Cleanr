...
## Agent 因扫描不完整而要求 macOS 完全磁盘访问权限

使用 Cleanr Skill 不需要授予完全磁盘访问权限。Safari、CloudKit 或临时目录
无法读取时，可以保留为未检查范围。先查看 `scan.issues`：仅凭报告标为
`partial`，无法认定缺少隐私权限。

Skill 可以继续总结已有证据；需要准备清理计划时，在已授权范围内尝试一次缩小
范围的扫描。跳过的位置会明确列出，自动预选使用新一轮完整扫描的证据。为这个
较小范围生成计划时，应保持相同的根目录、配置和修改时间年龄策略。局部扫描
完整不代表原始范围已经全部检查。

`plan` 和 `dry-run` 都是只读操作；权限造成的部分证据并不直接禁止查看计划，
但默认选择可能为空。如果 `scan.budget_exceeded` 非空，该结果不能生成计划；
应缩小扫描范围，而不是修改权限。真正清理时仍需审阅并确认确切计划。
