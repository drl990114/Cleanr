...
以自身 `?` 帮助为准。
下文的 `p`/`o`/`v`/`Tab`、视图快照复用和操作进度已在当前开发版本实现，尚待发布。
...
按 `p` 查找路径，忽略字母大小写，并接受正反斜杠；中文按原文匹配。
...
按 `Tab` 聚焦详情，使用方向键、Page Up/Down 或 Home/End 阅读长证据和完整路径。
终端宽度不足 88 列时，详情使用独立浮层。详情中的空格用于滚动，`Enter` 不改变选择；
`Tab` 或 `Esc` 返回列表。`?` 帮助也支持滚动。列表上方显示扫描范围和当前年龄门槛，
