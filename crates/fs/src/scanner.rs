...
use cleanr_core::{
    EntryKind, ReportIntegrity, ScanBudgetExceeded, ScanBudgetLimits, ScanEntry, ScanIssue,
    ScanIssueCode, ScanSummary,
};
...
use crate::{
    budget::ScanBudgetTracker,
    identity::{
        FileIdentity, HardlinkAccounting, HardlinkTracker, device_id_for_evidence, file_identity,
    },
    roots::ResolvedScanRoots,
};

#[cfg(test)]
use crate::identity::hardlink_identity;
...
pub(super) struct PendingHardlink {
    path: PathBuf,
...
pub(super) fn normalize_roots(mut roots: Vec<PathBuf>) -> Vec<PathBuf> {
